...
class View:
    ...
    def button2(self):
        button_2 = tk.Button(self.frame_up, text="Add to favourite", font=('Segoe UI',10), command=lambda: self.favourite_cities(self.entry_1.get()))
    ...
    def upper_frame(self):
        ...
        self.frame_up.place(relx=0.1, rely=0.05, relwidth=0.8, relheight=0.15)

    def lower_frame(self):
        self.frame_low = tk.Frame(self.root, bg='#ffff66')
        self.frame_low.place(relx=0.1, rely=0.3, relwidth=0.8, relheight=0.6)

    def favourite_cities(self):
        favourites_list = []
        clicked = StringVar()
        clicked.set('Favourites')
        self.favourites = tk.OptionMenu(self.root, clicked, *favourites_list)
        self.favourites.place(relx=0.1, rely=0.95, relwidth=0.2, relheight=0.05)
